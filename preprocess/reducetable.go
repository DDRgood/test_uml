...
func (reducer *ReduceTable) Reduce1() []float64 {
	...
	for {
		...
		//fmt.Println("Alpha 13 after1", reducer.alpha[130])
		if reducer.RemoveUnique {
			//delete unique
			for _, val := range reducer.alpha {
				if len(val) == 1 {
					//fmt.Println("Val = 1", val, "For key", key)
					//fmt.Println("Betta for val = 1:", reducer.betta[val[0]])
					...
					// delete from columnsKey ?????
				}
			}
		}

		//fmt.Println("Alpha 13 after2", reducer.alpha[130])
