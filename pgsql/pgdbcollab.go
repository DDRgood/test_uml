...
func (dbTool *DBTool) GetTableById(matrixId int) ([][2]int, []float64,
	map[int][]int, map[int][]int, error) {
	// open connection
	db := dbTool.GetConnection()
...
func (dbTool *DBTool) GetTableByName(matrixName string) ([][2]int, []float64,
	map[int][]int, map[int][]int, error) {
	// open connection
	db := dbTool.GetConnection()
	//try to close connection
...
func (dbTool *DBTool) AddMatrixFromFile(filePath string) {
	...
	dbTool.postTable(table, costs, len(alpha), len(betta), filename)
}

func (dbTool *DBTool) postTable(table [][2]int, costs []float64, numRows, numColumns int, name string) {
	// open connection
	connStr := dbTool.getConnStr()
	db, err := sql.Open("postgres", connStr)
	if err != nil {
