...
/*
	...
	DONT USE IT DELETE THIS SHIT



*/

func (bha *BHASolver) Solve1(limit float64, popSize int, numIter int, costs []float64,
	repair *scpfunc.SolutionRepairer, binarizer *supmath.Binarizer) (float64, []float64, float64, float64) {

	rand.Seed(time.Now().UnixNano())
	//	tmp := make([]float64, 0)
	//	tmp = append(tmp, costs...)
	//	tmp = append(tmp, limit)
	//	tmp = bha.getCost(tmp)
	costs_ := bha.getCost(costs)
	//limit = tmp[len(costs)]
	calcCollapse := bha.getCollapseFun()
	...
	for step := 0; step < numIter; step++ {
		...
		//new
		globalFitCur, _ := scpfunc.CalcFitness(globalBest, costs)
		//fmt.Println(globalFitCur)
		if globalFitCur <= limit {
			break
		}
	...
	globalFit, _ = scpfunc.CalcFitness(globalBest, costs)
	return globalFit, globalBest, Steps, Countmath
}
